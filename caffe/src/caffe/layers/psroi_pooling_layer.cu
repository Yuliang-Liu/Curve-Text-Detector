...
namespace caffe {
  ...
  __global__ void PSROIPoolingForward(
    const int nthreads, // R*2*7*7=98 or ...
    const Dtype* bottom_data, // bottom[0]
    const Dtype spatial_scale, // 0.0625
    const int channels, // bottom[0].c
    const int height, const int width, // bottom[0]. h w
    const int pooled_height, const int pooled_width, // 7 * 7
    const Dtype* bottom_rois, // bottom[1]
    const int output_dim, // confidence 2  cors 4
    const int group_size, // 7 * 7
    Dtype* top_data, // top[0] R*2*7*7 or R*8*7*7
    int* mapping_channel) {
    CUDA_KERNEL_LOOP(index, nthreads) { // one loop for one bin
      // The output is in order (n, ctop, ph, pw)
      int pw = index % pooled_width;
      int ph = (index / pooled_width) % pooled_height;
      int ctop = (index / pooled_width / pooled_height) % output_dim;
      int n = index / pooled_width / pooled_height / output_dim;  // R
      ...
      int gw = pw;
      int gh = ph;
      int c = (ctop*group_size + gh)*group_size + gw; // 0*7*7 + gh*7 + gw | top-left index for current bin of the top[0] 

      // roi_batch_ind * channels for each roi
      bottom_data += (roi_batch_ind * channels + c) * height * width; // choose channel for each bin of channels*7*7  
      Dtype out_sum = 0;
      for (int h = hstart; h < hend; ++h) {
        for (int w = wstart; w < wend; ++w) {
          int bottom_index = h*width + w; // top-left index of the current bin in the entire image.
          out_sum += bottom_data[bottom_index]; 
        }
      }

      Dtype bin_area = (hend - hstart)*(wend - wstart);
      top_data[index] = is_empty? 0. : out_sum/bin_area; // average
      mapping_channel[index] = c; // index described above
    }
  }
