...
void LSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  const Dtype dropout_ratio = this->layer_param_.recurrent_param().dropout_ratio();
  CHECK_GT(dropout_ratio, 0.) << "dropout_ratio must be > 0";
  CHECK_LT(dropout_ratio, 1.) << "dropout_ratio must be < 1";
